...
exports.updateOwner = async (req, res) => {
  try {
    ...
    res.send({
      error: true,
      message:
        "Stop tampering with the authentication tokens, it's a cyber crime!",
    });
  }
};
...
exports.updateMenu = async (req, res) => {
  try {
    ...
    await owner.findOneAndUpdate(
      { email: email },
      { menu: menu, price: price, lastUpdated: lastUpdated, menuType: menuType }
    );
...
exports.findMenu = async (req, res) => {
  try {
    const data = await owner.find({
      menu: { $regex: req.body.menu, $options: "i" },
    });
    let resArray = [];
...
exports.checkRate = async (req, res) => {
  try {
    ...
    const searchData = await customer.find({
      email: customerEmail,
      ratedMess: { $regex: ownerEmail, $options: "i" },
    });
